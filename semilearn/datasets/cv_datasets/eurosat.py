...
def get_eurosat(args, alg, dataset, num_labels, num_classes, data_dir='./data', include_lb_to_ulb=True):
    ...
    dump_dir = os.path.join(data_dir, args.dataset, "labeled_idx")
    ...
    ulb_dump_path = os.path.join(dump_dir, f'ulb_labels{args.num_labels}_{args.ulb_imb_ratio}_seed{args.seed}_idx_1.npy')

    if not os.path.exists(lb_dump_path) or not os.path.exists(ulb_dump_path):
            np.save(lb_dump_path, train_labeled_idxs)
            np.save(ulb_dump_path, train_unlabeled_idxs)
    ...
    if alg == 'priomatch':
        ...
        train_unlabeled_dataset = EuroSat(alg, data_dir, split="trainval", is_ulb=True, idx_list=train_unlabeled_idxs, transform=transform_weak, transform_medium=transform_medium, transform_strong=transform_strong, pred_label=pred_label, silhouette=silhouette)
...
class EuroSat(ImageFolder, BasicDataset):
    ...
    def __init__(self, alg, root, split, is_ulb=False, idx_list=None, transform=None, target_transform=None, transform_medium=None, transform_strong=None, pred_label=None, silhouette=None):
        ...
        if split == 'trainval':
            ...
            self.idx_list = list(range(0, len(self.targets)))
    ...
    def __sample__(self, index):
        ...
        if self.alg == 'priomatch' and self.is_ulb:
            pred_label = self.pred_label[idx]
            silhouette = self.silhouette[idx]
            return img, target, pred_label, silhouette
