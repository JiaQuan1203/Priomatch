...
class BasicDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        """
        if self.alg == 'priomatch' and self.is_ulb:
            img, target, pred_label, silhouette = self.__sample__(idx)
        else:
        ...
        if self.transform is None:
            ...
            if not self.is_ulb:
                ...
                if self.alg == 'priomatch':
                    return {'idx_ulb': idx, 'x_ulb_w': img_w, 'x_ulb_s': self.strong_transform(img), 'cluster_label_ulb':pred_label, 'silhouette_ulb':silhouette}
